...
import { ReturnParcelUseCase } from './return-parcel-use-case'
...
describe('Return Parcel Use Case', () => {
  beforeEach(() => {
  ...
  it('should be able to return a parcel', async () => {
    ...
    expect(inMemoryParcelsRepository.items[0]).toEqual(
      expect.objectContaining({ status: 5, returnedAt: expect.any(Date) }),
    )
  })
})
